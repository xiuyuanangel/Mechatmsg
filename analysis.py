...
def wordcloud_(wxid, start: str, end: str):
    import jieba
    wx = gmt.WeixinDbStorage()
    txt_messages = wx.get_msg_list_by_username(wxid, start, end, type='1')
    # txt_messages = [[item[0]] + item[-1].decode(errors='ignore').split(':\n')+[item[-2]] for item in txt_messages]
    # txt_messages = [[item[0],item[1],':\n'.join(item[2:-1]), item[-1]] for item in txt_messages ]
    txt_messages = [[item[0], item[-1].decode(errors='ignore'), item[-2]] for item in txt_messages]
    ...
    text = ''.join(map(lambda x: x[1], txt_messages))  # 1“我”说的话，0“Ta”说的话
    ...
    # 构建 FFmpeg 可执行文件的路径
    stopwords = set()
    stopwords_file = './resources/data/stopwords.txt'
    if not os.path.exists(stopwords_file):
        resource_dir = getattr(sys, '_MEIPASS', os.path.abspath(os.path.dirname(__file__)))
        stopwords_file = os.path.join(resource_dir, 'resources', 'data', 'stopwords.txt')
    with open(stopwords_file, "r", encoding="utf-8") as stopword_file:
    ...
    return {
        ...
        'keyword': keyword,
        'max_num': str(max_num),
        'dialogs': wx.get_messages_by_keyword(wxid, keyword, num=5, max_len=12)
    }
...
def wordcloud_christmas(wxid, start, end, year='2023'):
    import jieba
    wx = gmt.WeixinDbStorage()
    txt_messages = wx.get_msg_list_by_username(wxid, start, end, type='1')
    txt_messages = [[item[0], item[-1].decode(errors='ignore'), item[-2]] for item in txt_messages]
    ...
    text = ''.join(map(lambda x: x[1], txt_messages))
    total_msg_len = len(text)
    wordcloud_data = get_wordcloud(text)
    # return w.render_embed()
    keyword = wordcloud_data.get('keyword')
    max_num = wordcloud_data.get('keyword_max_num')
    dialogs = wx.get_messages_by_keyword(wxid, keyword, num=3, max_len=12,start=start, end=end)
    return {
...
def calendar_chart(wxid, start = '', end = ''):
    wx = gmt.WeixinDbStorage()
    calendar_data = wx.get_messages_by_days(wxid,start=start, end=end)
...
def month_count(wxid, start='', end=''):
    """
    每月聊天条数
    """
    wx = gmt.WeixinDbStorage()
    msg_data = wx.get_messages_by_month(wxid,start=start, end=end)
...
types = {'文本消息': 1,
    ...
    '位置消息': 48,
    '引用消息': 244813135921,
    '卡片式链接（带描述）': 17179869233,
    '卡片式链接': 21474836529,
    '小程序分享': 154618822705,
    '音乐卡片': 12884901937,
    '红包卡片': 8594229559345,
    '聊天记录合并转发消息': 81604378673,
    '拍一拍消息': 266287972401,
    '转账卡片': 8589934592049,
    '视频号直播卡片': 270582939697,
    '文件消息': 25769803825,
    '系统消息（撤回、加入群聊、群管理、群语音通话等）': 10000
 }
types_ = {1: '文本消息',
    3: '图片消息',
    34: '语音消息',
...
def sender(wxid, start = '',end='', my_name='', ta_name=''):
    wx = gmt.WeixinDbStorage()
    txt_messages = wx.get_msg_list_by_username(wxid, start, end)
    # txt_messages = [[item[0]] + item[-1].decode(errors='ignore').split(':\n')+[item[-2]] for item in txt_messages]
    # msg_data = [[item[0],item[1],':\n'.join(item[2:-1]), item[-1]] for item in txt_messages ]

    msg_data = [[item[0], item[1], item[-1].decode(errors='ignore'), item[-2]] for item in txt_messages]
    ...
    for message in msg_data:
        type_ = message[0]
        is_sender = 1 if message[1] == my_name else 0
        # subType = message[3]
        timestamp = message[-1]
    ...
    df = wx.get_usernames_by_nicknames()
    my_name_ = df[df['username']==my_name]['nickname'].values[0]
    ...
    p2 = (
        ...
        .add(
            "",
            [[my_name_, send_num], [ta_name, receive_num]],
            center=["40%", "50%"],
        )
        .set_global_opts(
            datazoom_opts=opts.DataZoomOpts(),
...
def my_message_counter(start, end, my_name=''):
    wx = gmt.WeixinDbStorage()
    msg_list = wx.get_msg_list_by_username(start=start, end=end)
    # msg_content_list = [[item[0]] + item[-1].decode(errors='ignore').split(':\n')+[item[-2]] for item in msg_list]
    # msg_content_list = [[item[0],item[1],':\n'.join(item[2:-1]), item[-1]] for item in msg_content_list ]

    msg_content_list = [[item[0], item[1], item[-1].decode(errors='ignore'), item[-2]] for item in msg_list]
    ...
    str_content = ''
    total_text_num = 0
    for message in msg_content_list:
        type_ = message[0]
        is_sender = 1 if message[1] == my_name else 0
        # subType = message[3]
        timestamp = message[-1]
        ...
        if weekday in weekday_count:
            ...
            weekday_count[weekday] = 1
        if type_ == 1:
            total_text_num += len(message[2])
            if is_sender == 1:
                str_content += message[2]
    receive_num = len(msg_content_list) - send_num
    data = [[types_.get(key), value] for key, value in types_count.items() if key in types_]
    if not data:
        return {
            'chart_data_sender': None,
